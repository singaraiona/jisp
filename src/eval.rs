...
macro_rules! eval_error { ($($x:expr),+) => { $crate::eval::Error(error_fmt!($($x),+)) }}
...
macro_rules! sym      { ($v:expr)          => { AST::Symbol($crate::eval::new_symbol($v.to_string())) } }
...
macro_rules! format_seq { ($l:expr) => {
    {
        let _suf = if $l.len() < $crate::eval::FMT_ITEMS_LIMIT { "" } else { ".." };
        format!("{}{}", $l.iter().take($crate::eval::FMT_ITEMS_LIMIT).map(|v| format!("{}", v))
